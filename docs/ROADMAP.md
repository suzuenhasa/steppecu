...
### Phase 2.5 — productization & remaining modalities  ← **largely DONE; see what's left**
The fit-engine **backend is finished** (Phase 2 above) **and it is now a product**: a 14-subcommand CLI + a Python facade ship over it. The build order followed the **backend-first sequence** (memory `build-sequence-backend-first`): step 1 (finish the backend) **DONE**; step 2 (productize what exists) **DONE**; step 3 (new statistics, each shipped with its own access surface) **DONE**. Prioritized from `docs/research/desirable-features-survey.md`:

- **STEP 2 — CLI + Python bindings + tidy (CSV/JSON) export — DONE.** The `app/` CLI (`src/app/`, CLI11) and the nanobind Python facade (`bindings/steppe`) both ship, wrapping the finished engine — qpAdm fit / `run_qpwave` / the S8 model-space rotation / the f2 precompute — with no R/CRAN dependency chain. The GPU-only scikit-build-core wheel was built + clean-venv-verified (`c935b28`). nanobind Python per ADR-0002.
- **STEP 3 — standalone f-stat entry points, EACH shipped WITH its own CLI/bindings — f4 / D-stat / f3 / outgroup-f3 / f4-ratio / qpDstat.** Exposing batched all-quartet / all-trio scans over resident `f2_blocks` is the highest-ROI new statistic (competes directly with Dsuite). Per the build sequence, each new statistic ships **together with** its access surface (not backend-only). Effort: LOW–MEDIUM (math) + the step-2 binding pattern.
  ...
  - **f4-ratio — DONE** (admixture proportion `alpha = f4(p1,p2;p3,p4)/f4(p1,p2;p5,p4)`): standalone `run_f4ratio` (`src/core/qpadm/f4ratio.cpp`), `steppe f4-ratio` CLI (`cmd_f4ratio.cpp`), `steppe.f4ratio` binding, `cli_f4ratio` ctest + 3 pytest gates, fixture-matched `golden_fit0_f4ratio_readf2.csv` (10 5-tuples). REUSES `assemble_f4_quartets` (ONE shared assemble over the interleaved num+den 2N-quartet array → one shared survivor-block set) + the `est_to_loo` replicate seam (no dup); the ONE new math seam is the per-block-RATIO weighted block-jackknife (AT2 `qpf4ratio` `jack_mat_stats $est`: `Rb = num_loo/den_loo`, `est = mean(tot−Rb)·nb + wmean(Rb,bl)`, ratio xtau), host-pure in the `.cpp` — NOT routed through `jackknife_cov` (its xtau decomposition differs). The SE is the jackknife OF THE RATIO statistic, not `se(num)/se(den)`. Bit-tight parity (alpha/se/z rtol 1e-6; golden independently fixture-reverified to ~1e-12, and the AT2 algorithm independently recomputed from the f2 fixture: `est` matches alpha to 2e-15 while `totnum/totden` would err by 3e-4 — the load-bearing `$est`-not-`tot` pin).
  - **D-stat / qpDstat — DONE** (Part A f2-path `a565ca0` + Part B genotype-path `5141ab0`): `steppe qpdstat` CLI (`--f2-dir` f4 OR `--prefix` normalized-D) + `steppe.qpdstat` / `steppe.dstat` bindings, golden-gated.
- **qpfstats — cross-statistic smoothing solve for heavy missingness — DONE** (`dd8f99a`). The documented AT2 remedy for aDNA's #1 false-positive driver; an over-determined regularized least-squares over the whole f2/f3/f4 system, on steppe's cuSOLVER + emulated-FP64 seam. `steppe qpfstats` CLI + `steppe.qpfstats` binding; max rel ~1.2e-12 vs the regenerated genotype golden over all 31995 entries.
...
- **Big-bet modalities — DATES + qpGraph BUILT; imputation deferred.** DATES / admixture dating (`4e54f9d`) and qpGraph (single-graph fit `cc9ff69` + topology search `5fe4c2c`) are BUILT — each with its CLI subcommand + Python facade + a real-AADR golden gate (see Phase 2.5 step 3 above and `docs/feature-matrix.md`). Still deferred: genotype imputation (`desirable-features-survey.md` §3).

> **Honesty guardrail.** Steps 2–3 ARE built (the CLI, the Python bindings, and the standalone f-stat / qpfstats / DATES / qpGraph entry points all ship and are golden-gated — see the per-feature commits above and `docs/feature-matrix.md`). The fit-engine *backend* is finished (F1–F6 git-verifiable: `e8430a2`…`2496a14`). The remaining gaps are release-engineering, not features: a rebuild-to-current + manylinux-portable wheel, CI, and the still-deferred format readers / precompute M6–M7 (see `docs/RELEASE-SCOPE.md`). steppe's throughput figures are its own internal numbers, never benchmarked against AT2, and always on real AADR (never synthetic).

### Phase 3 — Interfaces
Folded into Phase 2.5 **step 2** above (DONE): the CLI; the nanobind Python bindings; the scikit-build-core wheel (`c935b28`).
