...
- **M1 — Genotype decode + allele frequency.** GPU TGENO/PACKEDANCESTRYMAP decode (2-bit unpack) → per-pop Q/V/N, **with the pseudo-haploid path**. Replaces the numpy prototype.
- **M2 — Missingness + filters.** Pairwise-complete (V/N) default for parity; on-the-fly MAF / `geno` / `mind` / include-exclude.
- **M3 — SNP→block assignment.** `core/domain/block_partition_rule.hpp` (cM blocks, AT2 `blgsize`=0.05 Morgans default). Host-pure, shared by `io` and kernels.
- **M4 — Per-block f2.** The 3-GEMM kernel at **fixed-slice Ozaki (default 40-bit)**, **batched over blocks** → `f2_blocks [P × P × n_block]`. (This is the kernel from M0, now structured per-block — same precision findings apply.)
