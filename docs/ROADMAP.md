...
> **Substantially complete through M5.** M0–M4 (3-GEMM kernel → loader → filters → per-block tensor) + M4.5 multi-GPU + **device-resident output (`1f80c0c`)** + **M5 out-of-core streaming (`176a07d` tiered output + `c65179f` SNP-tile input)** are done and bit-identical. Full-autosome P=2500 completes on a single 32 GB 5090 in ~51.5 s. **The key lesson:** the precompute was host-result-bound — the perf win was getting the result OFF the CPU (device-resident output, ~4.3× at P=512) + streaming the input, **not multi-GPU.** Remaining: M6 (multi-dataset QC) + M7 (on-disk cache / FST / AT2 goldens). The **next real work is the Phase-2 fit engine.**
...
### Phase 2 — qpAdm FIT engine (operates on `f2_blocks`)  ← **THE NEXT REAL WORK (does not exist yet)**
Reads `f2_blocks` — **device-resident** for the in-VRAM case (small P; never bounce it through the host, `why-d2h.md`) and **streamed tiles** for large P (the M5 path). Block jackknife → SEs/covariance; f3/f4 derivation; qpWave rank test (SVD); qpAdm GLS + p-values; **multi-GPU MODEL ROTATION** — thousands of INDEPENDENT models, no combine, the embarrassingly-parallel phase that is **multi-GPU's proper home** (unlike the precompute). **Validation gate:** install R + admixtools and pin `extract_f2`/`qpadm` goldens (§12) — the AT2-parity gate for the fit outputs. Deferred/optional (after the fit exists): the TurboQuant-L2 rotation screen (`docs/research/turboquant-l2-experiment.md`).
...
> **AT2-parity gate is currently deferred (prerequisite not yet installed).** As of 2026-06-19 the GPU box has **no R and no `admixtools` package**, so ADMIXTOOLS 2 goldens do not exist yet. Until R + admixtools is installed and pinned goldens are generated (its own tracked task — small fixture, recording R version / `RNGkind` / AT2 version / `blgsize` / `boot` / seed per §12), the **operative gate for M1–M5 is the CPU reference oracle + property identities + internal-consistency + bit-identical-parity checks on real AADR** (never synthetic, §0). M1–M3 met this gate (decode reproduces the numpy oracle bit-for-bit; filters proven by drop-equals-mask + scalar-oracle exactness; block rule by the 757/719 internal-consistency counts); M4.5 + M5 by memcmp bit-identity (multi-GPU combine; streamed P=2500 vs in-core). AT2 parity is the hard acceptance criterion for the on-disk cache (M7) **and for the Phase-2 qpAdm fit outputs** (`extract_f2`/`qpadm` goldens), wired the moment the toolchain is present.
