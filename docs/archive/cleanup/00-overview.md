...
| B26 | MED | **✅ SATISFIED-BY-B5 (`faeb1f4`), gated by `vram_budget_unit` — no standalone B26 commit (the phase2 verdict was skipped; bookkeeping only, code correct; forensic audit `docs/cleanup/missing-fixes.md`).** §11.2 budget undercounts `vpair` 2× (X-13): the 2× term is live at `vram_budget.hpp:66` (`resident_tensor_bytes` = `2·P²·n_block·8`), `fstats.hpp` doc-comment counts both, the `architecture.md` §11.2 `Vpair` row is present. Do not let a future budget edit silently drop the 2×. | `vram_budget.hpp`, `fstats.hpp`, `architecture.md` §11.2 |
