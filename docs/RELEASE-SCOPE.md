...
3. **The wheel was built + clean-venv-verified ONCE (`c935b28`, the M(py-2) GPU-only wheel — a clean-venv `pip install` reproduced `golden_fit0` through the installed API + `extract_f2` on real AADR), but it has not been rebuilt to the current tree and the manylinux/auditwheel portability path is unverified** (no `[tool.cibuildwheel]`; the build was an ad-hoc `pip wheel .` on the box; no `auditwheel repair --exclude` of the CUDA SONAMEs run; platform tag / manylinux compliance unproven). This is the central distributable unknown. → Rebuild to current inside `nvidia/cuda:13-devel`, `auditwheel repair` with the CUDA-SONAME exclude list, install into a fresh CUDA-13 venv, re-assert `import steppe` + one real-AADR fit; codify as a cibuildwheel job. **[L]**
...
9. ~~**README is a 19-line stub**~~ → **RESOLVED (`db1110f`)** — rewritten into a real front door (requirements, install, the 14-command CLI table, Python quickstart, doc links + the input-format support matrix).
10. ~~**TGENO-only input limitation**~~ → **RESOLVED (Phase-2 format readers, `main @ e4a5387`).** steppe now reads **TGENO + PACKEDANCESTRYMAP/GENO + EIGENSTRAT + PLINK + ANCESTRYMAP** (auto-detected from the `--prefix` triple), each validated **bit-exact** against the TGENO decode on real AADR via `convertf` fixtures (no new statistical golden). `geno_reader.cpp:81` no longer throws; the README "Supported inputs" row is updated. This closes what was the #1 release blocker.
...
> **Also pre-release (operational, not a code artifact):** the **agentscripts/handoff IP-leak** (item P-1 below) is severity-elevated by one auditor to release-blocker because the (now-untracked) `agentscripts/` + `handoff-*.md` dev-tooling files carried literal vast.ai IPs/ports/SSH-key-paths (`root@<redacted-ip>` forms). For a public repo this is an info-disclosure smell. **RESOLVED (Phase-1 hygiene):** `agentscripts/` and `handoff-*.md` are untracked + gitignored and these in-doc IPs are redacted; the leak is closed going forward. **History-rewrite is still required before making the repo public** (the IPs remain in prior commits) — the *wheel* itself ships only `bindings/steppe` and is clean.
...
- **[BLOCKER, L]** Wheel built+verified once (`c935b28`) but not rebuilt-to-current / manylinux-unverified → §2 #3.
