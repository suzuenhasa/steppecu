...
| **M(fit-2) — rank test (S5) + p-values** | S5 SVD (`gesvdjBatched` when `nl,nr ≤ 32`, per-model `gesvd` `m>n` fallback otherwise); `χ²`, `dof`, `p`, qpWave rank sweep, nested rank-drop. | M(fit-1). | `chisq` tight, `dof` exact, `p`/rank decision loose, against AT2 `qpadm()$rankdrop` / `qpwave()` goldens [SPEC §12]. Both an `nr ≤ 32` and an `nr > 32` fixture exercised (batched vs fallback dispatch reported). |
