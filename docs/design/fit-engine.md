...
covered; `gesvda` is **excluded** (screening-only).
...
This is multi-GPU's decisive payoff (precompute sharding is "a modest throughput layer", measured
slower than single-GPU until the host bounce was removed; S8 is the linear-scaling win) [SPEC §0, §11.4].
...
- **Replication precondition.** `f2_blocks` must be resident on every device in `Resources::gpus`.
  ...
  cross-GPU traffic in S8, one-time, off the critical path [SPEC §11.4].
...
| **M(fit-2) — rank test (S5) + p-values** — **BUILT** (CPU oracle + GPU deliverable; nr≤32 GPU GATE green) | `rank_sweep` virtual (backend.hpp): the qpWave/qpAdm rank sweep r=0..rmax (`χ²`, `dof`, `p`), the AT2 `res$rankdrop` nested table (`f4rank`/`chisqdiff`/`dofdiff`/`p_nested`), the AT2 `res$popdrop` leave-one-LEFT-SOURCE-out table (verbatim `admixtools::drop_pops`: SUBSET the rows of the already-computed `f4_est` + SUBSET the FUDGED `qinv[ind,ind]`; NO re-gather/re-jackknife — the host orchestrator `ranktest.cpp` is backend-agnostic so the SAME path serves the CPU oracle AND the GPU deliverable), and `run_qpwave` (no target prepend). `rank_alpha` (default 0.05) names the rank decision (`f4rank` = smallest r with p>alpha). **GPU `CudaBackend::rank_sweep` (THE deliverable, BUILT): uploads x_total+Qinv ONCE, builds dXmat ONCE, then per r reuses the M(fit-4) device kernels (seed→ALS→weights+chisq) and reads back `chisq(r)` — the rank test RUNS ON THE GPU (f2 resident, on-device SVD/ALS/chisq, no host round-trip); the host forms dof/p/rankdrop/f4rank; `rank_Q` is `core::jacobi_svd(Q)` host-side (observability, bit-identical to the oracle).** **DISPATCH (SPEC §5:231) is REPORTED on `RankSweep.svd_path` (1=`gesvdjBatched` would-select when nl,nr≤32; 2=`gesvd` would-select) but the EXECUTED SVD is the deterministic on-device Jacobi at ALL sizes** — the cuSOLVER routing is the documented PENDING seam (re-routing would risk the green M(fit-4) bit-parity; the sweep needs `svd(X)` once per model so SVD is not the bottleneck). **SVD-once is an S8 throughput follow-on** (per-r recompute of the deterministic Jacobi is bit-identical; rmax=1 at the golden ⇒ ≤2 SVDs). | M(fit-1) + M(fit-4). | **GPU GATE green** vs `golden_fit0.json` `res$rankdrop`/`res$popdrop` (nr≤32 batched gate) RUNNING ON THE GPU: `f4rank`=1 EXACT, `dof`/`dofdiff` EXACT, `chisq`/`chisqdiff` tight (rtol 1e-6; measured |Δ|≤7e-11), `p`/`p_nested`/rank-decision loose (rtol 1e-3), rankdrop+popdrop tables row-for-row; GPU == CpuBackend oracle to 1e-9; `svd_path`=1 (gesvdjBatched would-select), `rank_Q`=10. CPU ORACLE also green at the SAME tiers (the diff localizer). nr>32 `gesvd` FALLBACK: golden PINNED — `golden_fit1_NRBIG.json` + `fixtures/f2_fit1_NRBIG.bin` (REAL AADR v66.p1_HO, same `geno_sha256` 7af8c2f5…, admixtools 2.0.10, R 4.3.3; target=England_BellBeaker, left=2, right=40 ⇒ **nr=39 > 32**, P=43, nb=701; `f4rank=1`). The golden rankdrop/popdrop were computed from the SAME `read_f2(outdir,pops=…)` f2 object the fixture is dumped from (`golden_fit1_generate.R`), so they are bit-consistent with the fixture ⇒ the **CpuBackend oracle reproduces them at the tight tier (chisq rtol 1e-6, dof/dofdiff/f4rank exact) — the nr>32 SWEEP MATH is GATED on the CPU oracle.** `svd_path` REPORTS `gesvd would-select` (2). **The nr>32 GPU sweep is the cuSOLVER-routing PENDING seam: the on-device transliterated LA kernels CANNOT be widened to nr=39 — CUDA reserves a kernel's per-thread LOCAL memory for the device's max resident-thread count, so a big-nr per-thread frame (`Wm[m·t]` grows with nr) trips `cudaErrorMemoryAllocation` at launch even single-threaded (MEASURED on box5090). `CudaBackend::rank_sweep` GUARDS nr>32 (the host caller reports the SKIP + the `gesvd would-select` dispatch); a VRAM-scratch kernel rewrite is the resident nr>32 follow-on.** [SPEC §12] |
...
| **M(fit-6) — S8 rotation, G==1 then G==2** | `run_qpadm_search`; `model_search_core` work-queue + re-sort + exception discipline; one-time `f2_blocks` broadcast; per-model graph capture + `search_streams` lanes (throughput follow-on). | M(fit-5). | `model_search_core` GPU-free unit test against a fake backend (queue/determinism); **bit-identical results AND identical ordering across G=1 vs G=2** [SPEC §11.4, §18]. |
