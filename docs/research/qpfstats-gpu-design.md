...
Status: IMPLEMENTED on `phase2-fit-engine` (run_qpfstats + CLI `steppe qpfstats` +
`steppe.qpfstats` binding + CpuBackend oracle + CudaBackend on-device solve). The genotype
front-end + the allsnps numerator engine are REUSED from qpDstat-B verbatim (no new genotype
machinery); the smoothing solve is the new on-device shared-factor batched least-squares.
Effort: MEDIUM-HIGH (on-seam, but more new code than f4/f3; ~55% reuse / 45% new).

## IMPLEMENTED — what landed + the as-built parity record

FILES: `include/steppe/qpfstats.hpp`, `src/core/stats/qpfstats.cpp` (the orchestrator),
`src/device/backend.hpp` (`QpfstatsSmooth` POD + `qpfstats_smooth` virtual),
`src/device/cpu/cpu_backend.cpp` (the native small_linalg oracle),
`src/device/cuda/qpfstats_kernel.{cu,cuh}` (zero-NaN-ymat + add-ridge-diag),
`src/device/cuda/cuda_backend.cu` (the syrk + potrf + gemm + Dtrsm-pair solve),
`src/app/cmd_qpfstats.{cpp,hpp}` + cli wiring, `bindings/module.cpp` + `bindings/steppe/__init__.py`.
TESTS: `tests/reference/test_qpfstats_smooth_parity.cu` (CpuBackend==CudaBackend on the solve
primitive, all NaN branches), `tests/cli/test_cli_qpfstats.cpp` + `tests/python/test_py_qpfstats.py`
(the genotype golden, element-wise).

THE BASIS (VERIFIED vs AT2 source, box5090): popcomb = f2 ∪ f3 ∪ f4 (for n=9: 36+252+378=666),
the design `x` is [npopcomb × npairs=C(n,2)=36] from `construct_fstat_matrix` (ASSIGNMENT — NOT
accumulation — of the ±1 f4-identity coefficients; the pure-f2 row collides on one column and
the second `=1` overwrites, then ×2 then ÷2 → 1.0; an accumulation `+=` was the first bug,
giving diag(x'x)=19.75 vs AT2's 16.75). numer[comb,block] = numsum/cnt (the dstat_block_reduce
rowMeans, f4mode → numerator-only, densum ignored). The solve is AT2 qpfstats_regression
reformulated: A_shared = x'x + ridge·I (ridge 1e-5), b[:,blk] = solve(A_blk, x'·ymat[:,blk]),
bglob = solve(A_y, x'·y); the NaN-comb-row downdate + the all-NaN→b=0 policy; recenter
f2blocks2 = f2blocks − f2(f2blocks)$est + bglob (block_lengths-weighted jackknife est).

PARITY: max abs diff 7.1e-15, max rel 1.2e-12 vs the AT2 golden over all 31995 entries
(9×9×711 upper-tri incl diag), the all-NaN block (0-based 535) matched as the recentered
constant. CpuBackend == CudaBackend ~1e-9.

CORRUPT-GOLDEN FIX (the AADR-TGENO-corrupt memory, CONFIRMED): the committed
`golden_qpfstats_geno.csv` was generated by AT2 v2.0.10 reading the RAW TGENO
(`raw/v66.p1_HO.aadr.patch.PUB`), which AT2 SILENTLY MISREADS (it gave Czechia Q=0.611 / cnt=222
for block 0 comb 0; steppe gives 0.1875 / 302). AT2 reading the convertf-PA
(`converted_pa/v66_HO_pa`, the TRUSTED format) gives EXACTLY steppe's 0.1875 / 302 /
ymat=0.01793898 — proving steppe's TGENO decode is correct and the old golden corrupt. The
golden was REGENERATED via `admixtools::qpfstats(converted_pa/v66_HO_pa, the 9 pops)` (the same
PA path the dstat/extract goldens use; bit-reproducible, run1==run2 max diff 0). The dstat
golden masked this because the normalized-D RATIO is invariant to the per-SNP Q discrepancy.
