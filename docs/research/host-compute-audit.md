...
> held (STEPPE_THOROUGH 62/62; CpuBackend==CudaBackend). STILL OPEN: **dates** — the two-row
> gather + target-record device repack (Stage 3, M5). Those host loops remain.
...
SNP-tiled (`1b36a0b`, the 40-pop genotype-f4 path 59 s → 8.1 s). Remaining: the dates
decode twin, M7 (wire the existing batched SE kernel into the single-model path), M5/M6 (dates).
