...
| Multi-GPU | **Single-node multi-GPU, day one** (single process, N devices) | The objective is parallelism + speedup **with parity** across the GPUs in one workstation. Single-process-multi-GPU (one host thread per device, per-device streams, NCCL for broadcast only — the parity reduce is host-side fixed-order, §11.4); multi-*node* remains deferred (§9, §11.4, §12, ADR-0009). **Honest role (MEASURED, M4.5/M5):** on the *precompute* multi-GPU is a **modest throughput layer** — the at-scale enabler is M5 device-resident output + streaming, not sharding (§11.1, §11.4). Multi-GPU's decisive payoff was *expected* in the **fit/rotation phase (S8)**; but **MEASURED (real AADR, P=600): the S8 rotation is host-bounce-capped on the consumer 5090s** — the one-time `f2` replication to each GPU is ~8.72 GB / ~3.8 s through host (no GeForce P2P), so G2/G1 only ~1.21× at 9086 real models. **Run the fit/rotation single-GPU on the 5090s;** multi-GPU rotation is deferred (`TODO(multigpu-host-bounce)`) — its payoff needs P2P hardware or per-device precompute (§5, §11.4). |
...
#        through M(fit-6), and the BACKEND-FINISH landed (F1-F6, 2496a14): M(fit-5) domain outcomes now BUILT+tested,
#        run_qpwave a tested first-class entry, NA/missing-block drop, Status::ChisqUndefined. The BACKEND IS FINISHED.
#        Still planned (step 2+): app/ CLI, Python bindings, standalone f3/f4/D-stat entry points, precompute M6/M7.
...
│       ├── error.hpp                    # [BUILT] Status enum (Ok/DeviceOom/RankDeficient/NonSpdCovariance/ChisqUndefined/
│       │                                #   InvalidConfig); the THREE domain outcomes = RankDeficient/NonSpdCovariance/
│       │                                #   ChisqUndefined (ChisqUndefined added by F3 ffdcba2: dof<=0 guard, was leaking NaN p)
...
│   │   │   │                            #   [F1 2496a14] ALSO core::pair_block_is_missing (Vpair==0 predicate): one source
│   │   │   │                            #   shared host/device, the AT2 read_f2(remove_na=TRUE) block-DROP test (not impute-0)
...
│   ├── reference/test_qpadm_parity.cu  # [BUILT, M(fit)] run_qpadm vs REAL-AADR AT2 golden_fit0/fit1_NRBIG, CPU+GPU
│   ├── reference/test_qpadm_rotation.cu # [BUILT, M(fit-6)/F6] S8 rotation vs golden_rot + G1==G2 FULL-QpAdmResult determinism (F6 360e386)
│   ├── reference/test_qpadm_domain.cu  # [BUILT, F2 c8fe397] M(fit-5) domain-outcome gate: degenerate REAL-AADR models ->
│   │                                   #   RankDeficient/NonSpdCovariance/ChisqUndefined as STATUS values (no crash/NaN), CPU+GPU
│   ├── reference/test_qpwave_parity.cu # [BUILT, F4 6481dfa] run_qpwave vs REAL AT2 qpwave() golden_qpwave, both backends
│   ├── reference/test_qpadm_missing_block.cu # [BUILT, F1 2496a14] NA/missing-block drop vs golden_fitNA (real maxmiss=0.99,
│   │                                   #   real Vpair==0 dropped block) + maxmiss=0 no-drop byte-identity arm, CPU+GPU
...
**Note on the fit engine (S3–S8): BUILT, golden-gated on the GPU, and the BACKEND IS FINISHED.** As of the M(fit-0..6) milestones (design `docs/design/fit-engine.md` milestone table; HEAD on `phase2-fit-engine` == `main`), the entire fit chain exists and the production CUDA-backend path is validated against **real-AADR AT2 goldens**: `tests/reference/goldens/at2/golden_fit0.json` (9-pop, `nr≤32` small path), `golden_fit1_NRBIG.json` (`nr=39` cuSOLVER `gesvd` LARGE path), `golden_rot.json` (84-model rotation), `golden_qpwave.json` (a real `qpwave()` run), and `golden_fitNA.json` (real-AADR `maxmiss=0.99`, one real dropped block), all from admixtools 2.0.10 / R 4.3.3 / v66.p1_HO. The `CpuBackend` is the native-FP64 oracle the GPU is diffed against (run under `STEPPE_THOROUGH`). The orchestration lives in `core/qpadm/` (host-pure, layering-legal); the device overrides (`assemble_f4`/`jackknife_cov`/`rank_sweep`/`gls_weights`/`fit_models_batched`) live in `src/device/cuda/cuda_backend.cu` with kernels in `src/device/cuda/qpadm_fit_kernels.cu`. The public C++ entry points are `steppe::run_qpadm` / `steppe::run_qpwave` / `steppe::run_qpadm_search` in `include/steppe/qpadm.hpp` (a model references populations by **index** into the resident `f2_blocks` P axis — the GPU-first compute seam, no strings); `run_qpwave` is now a tested first-class entry — its no-target-prepend / `left[0]`-is-reference semantic is gated on both backends against `golden_qpwave` (F4 `6481dfa`, `test_qpwave_parity.cu`). **Per-model domain failures are returned as `status` values, not crashes — and M(fit-5) is now BUILT and tested as THREE outcomes (`RankDeficient`, `NonSpdCovariance`, `ChisqUndefined`)**: `ChisqUndefined` (F3 `ffdcba2`) closes a `dof≤0` model silently leaking a NaN `p` with `status==Ok`, guarded on both the host and the CUDA model-batched path; `test_qpadm_domain.cu` (F2 `c8fe397`) asserts all three on degenerate REAL-AADR models on both `CpuBackend` and `CudaBackend`. **NA / missing-block handling (F1 `2496a14`):** steppe's resident f2 is **pairwise-complete** (not the AT2 `maxmiss=0` global intersection), so a per-pair `Vpair==0` block CAN occur on sparse AADR and was being silently imputed `f2=0` (bias toward 0); it now follows AT2 `read_f2(remove_na=TRUE)` — **DROP** any partially-covered block before the LOO/jackknife — via the single shared host/device predicate `core::pair_block_is_missing` (`f2_estimator.hpp`), on `CpuBackend` and the GPU (`f2_block_keep_kernel`, single-model and S8 survivor-compaction). The legacy `maxmiss=0` goldens stay BYTE-IDENTICAL (no-drop identity arm). The opt-in jackknife-SE policy (`JackknifePolicy{None,FeasibleOnly,All}`, default `All`) lets the rotation pay the expensive LOO SE only for the survivors worth reporting (M(fit-3 SE-policy)). The dead public `QpAdmOptions::constrained` field was removed (F5 `e8430a2`, API hygiene — non-negative constrained weights is a deferred step-3 feature, not shipped). **What is NOT yet built (the honest NEXT, step-2 productization): NO CLI, NO Python bindings, NO standalone f3/f4/D-stat entry points** (no `app/` or `bindings/` dir, no nanobind/CLI11 in CMake) — these stay "(planned)" below. **Multi-GPU caveat:** the S8 rotation is genuinely batched and shards across GPUs, but on the no-P2P consumer 5090s the one-time `f2` replication is a ~8.72 GB / ~3.8 s host bounce — only ~1.21× at 9086 real models — so **run the rotation single-GPU**; G≥2 is deferred (`TODO(multigpu-host-bounce)`, §11.4).
...
**Block-size unit (verified against ADMIXTOOLS 2 semantics).** ADMIXTOOLS 2's `blgsize` default is `0.05` **Morgans** = **5 cM**. The user-facing `--blgsize` CLI flag (and the Python `blgsize=` kwarg) speaks **Morgans**, the AT2 convention — a bare `--blgsize 0.05` reproduces AT2's block partition. The merge seam (`ConfigBuilder::build()`) converts Morgans → cM (× `kCentimorgansPerMorgan`) into the cM-valued `RunConfig::blgsize_cm()` field; the block math then converts cM → Morgans at the single `block_size_cm_to_morgans` site next to `block_partition_rule.hpp` before calling `assign_blocks` (which speaks Morgans). Two conversion sites, both single-homed; the only unit the *user* sees is Morgans. Do not conflate the layers.
...
2. **Model-space search (S8): embarrassingly parallel, zero cross-GPU traffic *per fit* — but the one-time `f2` replication is NOT free on no-P2P cards.** With `f2_blocks` replicated, models are partitioned across devices via a dynamic atomic work-queue over model indices (load-balances uneven per-model cost). **MEASURED (real AADR, P=600):** getting `f2_blocks` onto each GPU costs ~8.72 GB / ~3.8 s through host on the consumer 5090s (no GeForce P2P), which caps the rotation at ~1.21× at 9086 real models (no 1.5× crossover) — **run the rotation single-GPU on the 5090s** (multi-GPU deferred, `TODO(multigpu-host-bounce)`; eliminate via P2P hardware or per-device precompute — each GPU builds its own f2, zero cross-GPU transfer). Each fit runs wholly on one device, so its internal reductions stay single-GPU and inherit single-GPU parity; results are returned to the host and **re-sorted by model index**, so the final ordering is deterministic regardless of which GPU produced which model.
...
- **cuSOLVER: deterministic mode, with its real scope.** `cusolverDnSetDeterministicMode(handle, CUSOLVER_DETERMINISTIC_RESULTS)` is pinned **once** in the `CusolverDnHandle` ctor (`src/device/cuda/handles.hpp`), immediately after `cusolverDnCreate`, on the single dense-cuSOLVER handle the process holds (sticky handle state, mirroring the cuBLAS math-mode idiom). The CUDA 13.x default is **already** `CUSOLVER_DETERMINISTIC_RESULTS` (verified against `cusolver_common.h`'s `cusolverDeterministicMode_t` enum and the cuSOLVER docs, "By default, cuSolverDN computes deterministic results"), so the pin is **defensive** — it forecloses a future toolkit default flip silently de-determinizing the rank-test SVD, with no behaviour change today (the goldens do not move). Per the cuSOLVER docs the mode covers `geqrf`, `syevd`/`syevdx`, `gesvd` (for `m > n`), **`gesvdj`/`gesvdjBatched`**, `Xgeqrf`, `Xsyevd`/`Xsyevdx`, `Xgesvd` (`m > n`), `Xgesvdr`, and `Xgesvdp` — and **explicitly does NOT** cover `potrf`/`potri`/`getrf`/`getrs` (the SPD `Qinv` Cholesky and the ALS/weight/LOO LU solves), which therefore stay native-FP64 and **outside** the deterministic set. **Scope, precisely.** The mode governs **only** the loose-tier large/NRBIG rank-test SVD (`large_svd_V`, `cuda_backend.cu`): `gesvdj` when both dims ≤ 32 (covered unconditionally) and `gesvd` otherwise — for which the orientation rule (`rows = max(nl,nr) ≥ cols = min(nl,nr)`, `cuda_backend.cu:4221-4226`) already satisfies the `m ≥ n` precondition the `gesvd` family needs. The **primary bit-exact 9-pop golden's** rank-test SVD is **not** cuSOLVER at all: it is a **custom on-device fixed-order Jacobi** (`launch_qpadm_rank_via_jacobi` / `dev_jacobi_svd_V`), deterministic **by construction** (a fixed sweep/rotation order), so `cusolverDnSetDeterministicMode` is irrelevant to it and it stays byte-identical. We do **not** claim blanket "deterministic cuSOLVER"; we claim the enumerated set, and the GPU reference lane's `handles_unit` test (`tests/reference/test_handles.cu`) asserts the promise literally — post-construction `CusolverDnHandle::deterministic_mode()` returns `CUSOLVER_DETERMINISTIC_RESULTS`, and the rank-test routine is provably in the covered set (`gesvdj` always; `gesvd` because the orientation rule guarantees `m ≥ n`).
