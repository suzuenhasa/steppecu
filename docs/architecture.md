...
The single load-bearing domain insight that shapes everything: ADMIXTOOLS 2 is **precompute-once / fit-many**. One expensive, bandwidth-bound genotype pass produces a tiny, cacheable `f2_blocks` tensor (`[n_pop × n_pop × n_block]`, MB-scale); thereafter every model fit is small dense linear algebra over that resident tensor, batched across a block (jackknife) axis and a model axis. The architecture mirrors this split: a **precompute engine** (S0–S2) and a **fit engine** (S3–S8). The crucial corollary the rest of the document keeps returning to: the *output* (`f2_blocks`) is small, but the *input* (the genotype matrix) routinely does **not** fit in VRAM, so the precompute engine is an out-of-core streaming computation, not a load-it-all kernel (§11.1).
...
- A *precompute-once / fit-many* engine: one **out-of-core** streaming genotype pass, then batched small dense linear algebra over a resident tensor.
...
- A multi-*node* HPC framework. **Single-node multi-GPU is in scope and central, day one** — parallelism + speedup across the N devices of one workstation, *with* numerical parity, is a primary objective (§9, §11.4, §12). It is single-*process*-multi-GPU (one host thread per device, per-device streams, NCCL for the broadcast). Out-of-core genotype streaming remains a v1 requirement (§11.1) and now shards SNP tiles across the GPUs. The deferred boundary is **multi-node** (multiple processes / MPI / a launcher). **cuSOLVERMp stays deferred** — not for lack of multi-GPU ambition, but because qpAdm's `Q`/`X` are tiny: the right parallelism is *across many independent small fits* (S8), not *within one distributed factorization*; a distributed solve of a low-double-digit matrix is pure overhead and worse for determinism (§11.4).
...
| **S2 f2 + block accumulate** | `device/cuda/f2_block_kernel.cu`; assembled by `core/fstats/f2_from_blocks.cpp` | `ComputeBackend::accumulate_f2` | afs → `f2_blocks [n_pop × n_pop × n_block]` + per-block pairwise valid-SNP count `Vpair`, bias-corrected, via the **3-GEMM reformulation** below, batched over blocks | three batched **native-FP64** GEMMs + a fused elementwise pre-pass and a tiny native-FP64 cancellation step, reduction into block bins via the shared `block_partition_rule` | compute + bandwidth, **streamed, runs once** |
...
The precompute pass is bandwidth-bound *and out-of-core*: real EIGENSTRAT/PLINK datasets are 1M+ SNPs × thousands of samples (tens of GB of packed genotypes), far exceeding any single GPU's VRAM, while the `f2_blocks` output is MB-scale. We therefore **never load the genotype matrix whole.** The mechanism:
...
3. **Block accumulator is tiny and resident.** `f2_blocks` is `[n_pop × n_pop × n_block]` and stays on the device across the whole stream and the entire downstream model search; tiles never touch it except to add their contribution via the shared `block_partition_rule`.
4. **`afmat`/`countmat` sizing.** These are per-tile `[T × n_pop]`, *not* `[SNP × n_pop]` — they scale with the tile, never the dataset. If even `[SNP × n_pop]` were needed whole (it is not, given streaming), the same tiling applies.
...
Because cross-GPU traffic is tiny (kB `f2_blocks` broadcast) and the search phase has **zero** inter-GPU traffic, steppe is largely insensitive to NVLink-vs-PCIe — it is viable on commodity multi-GPU workstations; the streaming build is bound by host→device ingest and disk, not inter-GPU links. **cuSOLVERMp stays deferred** (§1): it distributes one *large* dense factorization across GPUs, but qpAdm's `Q`/`X` are low-double-digit — the right parallelism is across many independent small fits (above), not within one distributed solve, which would be pure overhead and worse for determinism. **Multi-node** (multiple processes / MPI) is the remaining deferred boundary.
