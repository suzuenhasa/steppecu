...
**Big picture (2026-06-18):** the **precompute half exists** — M0–M4 done (validated 3-GEMM f2 kernel → data front-end → filters → per-block `f2_blocks` tensor) **+ the entire before-M4.5 cleanup (B1–B27)** **+ M4.5 single-node multi-GPU (correct, bit-identical, AND faster: 1.10×@P=768, 1.22×@P=400, `867a4bf`)**. M0–M4 + cleanup merged to `main` @ `1fbb417`; branch `m4.5-multigpu` (46 commits ahead) is **being merged to `main` now** — M4.5 milestone complete. The **fit engine (Phase 2) does not exist yet**, and the precompute still has scale/robustness milestones (M5–M7). **The perf rabbit-hole is CLOSED. Next = the qpAdm FIT ENGINE (Phase 2)**, then M5 out-of-core streaming, AT2 goldens (M7 gate), M6 merge.
...
**Perf — RESOLVED, the rabbit-hole is CLOSED (`867a4bf`).** Root cause (nsys, `docs/cleanup/m4.5/why-multigpu-slow.md`, `165f655`) was the **data-bounce wart**: `compute_f2_blocks` D2H-copied each partial to host and freed its device buffers, forcing the P2P combine to re-upload (H2D) + place-add + a 2nd D2H of the full 7.14 GB. The **device-resident combine** kills it (partials stay resident; one D2D root copy + `cudaMemcpyPeer` per peer into a disjoint slice + one final D2H). Measured G2 vs G1: **1.10× @P=768, 1.22× @P=400**.
- ~~Refuted (do NOT carry as open problems):~~ the "per-call `cudaMalloc`/`cudaFree` driver-lock serializes the device threads / only ~18% overlap" hypothesis — nsys measured **74% overlap**; the L4 pool-allocator is at most a few % and is **NOT** the speedup lever (demote to OPTIONAL cleanup, not a blocker). The "~1.4 s combine host-zeroing (P1 default-init allocator) dominates" hypothesis — the accumulator memset is **5 ms**; P1 was REVERTED and is not the cure. `docs/cleanup/m4.5/perf-discovery.md`'s P1 premise is superseded by `why-multigpu-slow.md`.
- [ ] **[optional]** Pin the final pageable result D2H — the next available speedup, could push past 1.10×. Not a blocker.
- [ ] **[optional]** Bench byte-traffic columns are observability-only (currently print 0.00).
- [ ] **[optional / reassess]** B2 P2P transport rework (`agentscripts/m4.5-b2-p2p-fix-pass.js`) — its motivation (the bouncing combine) is largely addressed by the resident combine; **likely-subsumed**, not a pending speedup.
...
- [ ] **[adopt now] Nsight Systems measure-first gate** (§11.3) — classify ingest/launch/orchestration-bound before building any perf lever (see ⚡ section). The prerequisite that orders the perf work. **Already applied for M4.5** (nsys diagnosed the data-bounce wart → `867a4bf`, `docs/cleanup/m4.5/why-multigpu-slow.md`); still the gate for M5/Phase-2 levers.
