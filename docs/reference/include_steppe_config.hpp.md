...
| `kResidentHostInputStacks` | `3` | The number of dense host-side input stacks the resident output tier must hold: the three P×M_kept arrays (genotype, variance, and count — Q, V, N) it re-decodes into one host buffer and feeds to the resident engine. The tier-selector multiplies this by P × M_kept × 8 bytes (one `double` each) to size the host-RAM headroom the resident dense-decode buffer needs, and will not choose the resident tier unless that much host RAM is free. This host-input clamp is part of the extract-f2 out-of-memory fix: a model whose dense Q/V/N would overflow host RAM is routed to the streamed host-RAM or disk tiers — which re-decode each SNP tile on the fly instead of building the full dense buffer — rather than being allowed to over-budget and get killed. |
