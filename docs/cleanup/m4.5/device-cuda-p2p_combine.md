...
> **STATUS: the headline perf findings (P1/P2) are RESOLVED (@`867a4bf`).** This review was written against
> the PRE-FIX code. Its central critique — the host→peer→root **double-bounce** (P1) and "keep the partial
> device-resident so `cudaMemcpyPeer` is a *genuine* pull" (P2) — is exactly what landed: the
> **device-resident combine** (`867a4bf`) makes per-device compute leave its partial RESIDENT (a move-only
> `DevicePartial`), then the combine allocates ONE root result, D2D-copies the root partial +
> `cudaMemcpyPeer`s each peer partial straight into its DISJOINT block slice, then does ONE final D2H —
> deleting the H2D re-upload, the accumulator memset, the `place_add_f2_kernel`, the staging buffers, and
> the per-peer `cudaDeviceSynchronize`. The data-bounce wart (a redundant SECOND full 7.14 GB D2H) was the
> nsys-diagnosed root cause (`why-multigpu-slow.md` @`165f655`); after the fix **multi-GPU is FASTER than
> single-GPU** — measured on rtxbox (2× RTX PRO 6000 Blackwell sm_120, Release, EmulatedFp64{40}, median of
> 10): P=768 = **1.10×**, P=400 = **1.22×**, with bit-identical (`memcmp`) parity preserved on both combine
> paths and both datasets. The PRO 6000 box (`rtxbox`) is where that speedup and the nsys trace were
> measured — it is NOT "dark." The detailed findings below are kept as the record of the pre-fix code and
> the design rationale that drove the fix; treat P1/P2 (and the per-partial sync, the inline kernel that
> was deleted) as **LANDED**, not pending.
...
The problems concentrate on the **performance** axis the workflow flagged, plus a cluster of **idiom/contract** gaps the first pass missed: the unit (a) does a documented host→peer→root **double-bounce** for data that is already host-resident, (b) hard-`cudaDeviceSynchronize`s per peer partial, (c) re-enables peer access on every loop iteration (and ignores the `enable_peer_access` knob entirely — N2), (d) launches a non-grid-stride monolithic kernel **directly inline** rather than behind the project-wide narrow `void launch_xxx` wrapper (N1, §7), and (e) carries a comment that factually misstates `cudaMemcpyPeer`'s host-sync behavior (N3). None break parity. The honest counterweight — and the reason this is not lower — is that arch §11.4 itself classifies this combine as *"architectural cleanliness, not a throughput lever — steppe is deliberately P2P/NVLink-insensitive."* **[UPDATE post-`867a4bf`:** the device-resident rework of this path HAS since been exercised and measured on the P2P-capable tier (`rtxbox`, 2× RTX PRO 6000) — `memcmp`-identical to single-GPU AND **1.10×/1.22× faster** — so it is no longer "dark"; the pre-fix idiom findings below were the basis for that rework.] But a senior engineer would not sign off at 9+ on the file literally named "the prime perf target" while it violates the §7 streams/events, grid-stride, narrow-launch-wrapper, and async-pinned idioms this directly, and leaves a documented config knob unwired.
...
**[T2 — MED, PARITY-SAFE: yes] No DIRECT, box-independent unit test exercises the P2P combine in isolation; the parity test's P2P assertions are PRO-tier-gated, so on the consumer 5090 box (P2P disabled) the transport arm does not run** *(it DOES run on the PRO `rtxbox` tier — see the post-`867a4bf` update below)*. *(CONFIRMED)* I grepped `tests/`: the only reference to `combine_f2_partials_p2p`/`place_add` is the parity test, whose P2P arm runs only `if (resG2p.gpus[0].caps.can_access_peer)` (test lines 401, 428, 486–488). On the no-peer tier the routine degrades to host-staged in the caller, so the *transport* (`cudaMemcpyPeer`, peer-enable) is exercised only on PRO hardware. **[UPDATE post-`867a4bf`:** the PRO tier (`rtxbox`) HAS since run the parity gate AND the bench on the device-resident path — `memcmp`-identical and 1.10×/1.22× faster — so the transport is no longer untested; the double-bounce + per-partial sync this line cited were *deleted* by that rework. The residual gap is only that the gate is still PRO-tier-gated and there is no *box-independent* unit test.] The *combine arithmetic* is covered (host-staged sibling is byte-identical), but a transport-independent unit test of the resident D2D + `cudaMemcpyPeer` + the fixed-order loop would let the combine logic be tested on any box. The workflow notes the tier-assertions being PRO-specific is "a separate concern," so flagged not heavily scored.
