...
- **`main`** @ `2496a14` (branch `phase2-fit-engine` tracks it; the tip `32075c7` is an agentscripts-only workflow commit — no source). **Phase-1 PRECOMPUTE (S0–S2) is COMPLETE THROUGH M5** (device-resident output + M5 streaming) and **Phase-2 the qpAdm FIT ENGINE (S3–S8) is BUILT and golden-gated ON THE GPU.** The big-refactor is COMPLETE (below). **The fit-engine BACKEND is now FINISHED** — the 6 FINISH-NOW items from `docs/design/fit-engine-finish-punchlist.md` all landed (below). Parity is `memcmp` bit-identical / within tolerance on real AADR throughout.
...
1. **STEP 2 — PRODUCTIZATION (NOT BUILT — the next work).** There is **no user-facing surface yet**: no **CLI**, no **Python bindings** (no `app/` or `bindings/` dir, no nanobind/CLI11 in CMake, no `main()` — confirmed). `architecture.md` still marks `app/` + the standalone f-stats "(planned)". Today "running" steppe = the test-harness binaries + the C++ library API (`run_qpadm` / `run_qpadm_search` / `run_qpwave` in `include/steppe/qpadm.hpp`). Building the CLI + Python bindings is the next lever for actual use.
