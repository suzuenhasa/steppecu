...
- **`main`** @ `bc0b773` (branch `phase2-fit-engine` may sit a few commits ahead — verify with `git rev-parse --short main phase2-fit-engine`; a golden-regen workflow may be in flight). **Phase-1 PRECOMPUTE (S0–S2) COMPLETE THROUGH M5**; **Phase-2 qpAdm FIT ENGINE (S3–S8) BUILT + golden-gated**; big-refactor COMPLETE; **fit-engine BACKEND FINISHED** (F1–F6 below); **and FULL AT2 END-TO-END PARITY achieved** (the parity-chain block below). **ctest 45/45.**
...
- **STEP 2 — PRODUCTIZATION (CLI):** steppe is **runnable from the command line**.
  ...
  - **M(cli-4)** (`74c3c71`, recovered + re-gated) — `steppe extract-f2 --prefix … --pops … --out DIR` (genotypes → f2 dir; STPF2BK1 writer w/ real vpair; `--blgsize` Morgans, `--maxmiss`, `--ploidy auto|1|2`, `--drop-mono`). The full `extract-f2 → qpadm` study flow works. **Remaining CLI:** M(cli-2) `qpwave`, M(cli-3) `qpadm-rotate` → then M(py-1) bindings. Contract: `docs/design/cli-bindings.md`.
...
Backend FINISHED + **full AT2 end-to-end parity achieved** (the 4-fix chain, above) + **Haak 2015 reproduced**. Remaining:
0. **GOLDEN REGEN (likely IN FLIGHT or just-done).** The last cleanup: regenerate `golden_rot` / `golden_fit1_NRBIG` / `golden_fitNA` from the convertf-PA (they were the corrupt TGENO-misread; `golden_fit0`+`golden_qpwave` already correct) so the WHOLE suite validates against correct goldens. Workflow `agentscripts/regen-goldens-convertf.js` (run `wm9bhrf3m`); resumable on 529. **Check if it committed** (`git log --oneline -5`); if it HALTed, resume it. After this, all 5 goldens correct.
1. **STEP 2 — PRODUCTIZATION (CLI mostly done).** Built: M(cli-0) scaffold, M(cli-1) `qpadm`, M(cli-4) `extract-f2` (the full study flow works). **Next:** M(cli-2) `qpwave` → M(cli-3) `qpadm-rotate` → **M(py-1) Python bindings** (nanobind + DLPack seam — `docs/design/cli-bindings.md` + `docs/research/interop-usecases.md`). GPU-only (no CPU runtime).
