...
1. **qpAdm FIT ENGINE (Phase 2, S3–S8) — ACTIVE, in design** (`agentscripts/fit-engine-design.js` → `docs/design/fit-engine.md`). Does not exist yet. Reads `f2_blocks` (device-resident in-VRAM, or streamed tiles for large P), runs S3/S4 (f3/f4 derivation + jackknife), S5 (rank test / SVD), S6 (GLS solve — Cholesky + weighted normal equations), S7 (block-jackknife SE), S8 (the **model rotation** — many independent models = the embarrassingly-parallel, **multi-GPU-friendly** phase, multi-GPU's proper home). Build order + the first-milestone contract come from the design.
