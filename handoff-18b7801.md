# steppe — cold-start handoff @ `m4.5-multigpu` `18b7801`

Everything a fresh session needs to resume. Canonical spec: `docs/architecture.md`; build order + status: `docs/ROADMAP.md`; living task list: `docs/TODO.md`; the per-file audit + master cleanup backlog: `docs/cleanup/00-overview.md`; reusable workflow scripts: `agentscripts/`. **Read those before writing code.**

---

## 0. TL;DR
`steppe` = GPU/CUDA-13/Blackwell (sm_120) reimplementation of ADMIXTOOLS 2 f-statistics (precompute-once / fit-many), senior-engineer quality bar. **Status: Phase 1 precompute M0–M4 + the before-M4.5 cleanup (B1–B27) + M4.5 single-node multi-GPU are DONE.** M0–M4+cleanup merged to `main` @ `ac0ca6f`; **M4.5 is on branch `m4.5-multigpu` (8 commits `d81d2a1`→`18b7801`, 30/30 ctest green, clean from-scratch) — NOT yet merged to main.** **NEXT: (a) merge `m4.5-multigpu` → main; (b) the "5090 after" consumer graceful-degrade validation on the cheap vast box; then M5 (out-of-core streaming) / M7 (AT2 goldens).**

## 1. Boxes
- **CAPABLE box = rtxbox (Verda):** `ssh rtxbox` → `ssh -i ~/.ssh/id_vastai root@31.22.104.224` (port 22, same key as vast). 2× **RTX PRO 6000 Blackwell** (sm_120, 96 GB ea), CUDA 13.0.88, driver 580. **$3.78/h — expensive; spin down when idle.** Repo at `/workspace/steppe`; data `/workspace/data/aadr/{raw, derived_acc P=50, derived_full P=768}`. **GOTCHA: `nvcc` not on PATH → `export PATH=/usr/local/cuda/bin:$PATH` before cmake.** **Real stock-driver P2P confirmed: `cudaMemcpyPeer` byte-exact 55.6 GB/s, `can_access_peer==true`.** See [[rtxbox]].
- **BUDGET box = vast 2× RTX 5090** (`ssh -i ~/.ssh/id_vastai -p 43215 root@78.92.24.57`, ephemeral — may be down) — the consumer graceful-degrade TEST TARGET (P2P driver-disabled). See [[steppebox5090]].
