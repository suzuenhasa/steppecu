...
const fixer = await tryAgent([
  ...
  'STEP 1: read the NEW run_f4_sweep/run_f3_sweep signature (steppe/fstat_sweep.hpp + the .cpp) + SweepRequest; read the cmd_f4.cpp/cmd_f3.cpp current (explicit-list) structure; mine the CLI flag/dispatch plumbing from wip/fstats-massive-overbuild (git show). STEP 2: ADD the sweep flags (--all-quartets/--all-triples, --min-z [default 3.0], --top-k, --sure, --shard-dir) to cmd_f4/cmd_f3 (+ qpdstat if trivial), build a SweepRequest, dispatch to the NEW run_f4_sweep/run_f3_sweep; keep the explicit-list path byte-identical. STEP 3: ' + RSYNC + ' + ' + BUILD + ' — clean build + STEPPE_THOROUGH ctest green + the explicit goldens EXACT. STEP 4: WALL-CLOCK the full sweep on box5090, single-GPU --device 0 (construct the exact SSH command yourself, mind the quoting): run /usr/bin/time -v ./build-rel/bin/steppe f4 --all-quartets --f2-dir ' + F2_500 + ' --min-z 6 --sure --shard-dir /tmp/sweep_out (the full C(500,4)=2,573,031,125-quartet sweep; --min-z 6 keeps the output small but the wall-clock measures enumerating+computing+filtering ALL 2.57B), while in the BACKGROUND sampling nvidia-smi --query-gpu=utilization.gpu,power.draw,memory.used --format=csv,noheader -l 1 to a log to RECONFIRM GPU-bound. Capture: the wall-clock (the time -v elapsed/wall-clock line), quartets/sec = 2573031125 / wall_seconds, the GPU util range, peak VRAM + peak RSS (time -v Maximum resident set), the survivor count. Report all of that + the FULL ctest. Do NOT commit. NON-trivial blocker -> STOP + report.',
