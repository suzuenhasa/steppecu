...
const at2Run = await tryAgentRun([
  ...
  'DO: in R (Rscript, DETACH+poll): `extract_f2("' + PA + '", "/workspace/data/haak/at2_f2", pops=c(<union>), blgsize=0.05, maxmiss=0, overwrite=TRUE, n_cores=8)` then for EACH model `qpadm("/workspace/data/haak/at2_f2", target=<T>, left=c(<a,b[,c]>), right=c(<outgroups>), boot=FALSE)` and capture res$weights (est/se/z), the p-value/rankdrop, the SNP count. Report each model\u0027s full AT2 result. REAL DATA only; this is the correctness reference steppe is compared against.',
...
const report = await tryAgentRun([
  ...
  'WRITE ' + OUT + ' with: (1) the model table per model: target | sources | steppe weights[se]/p/feasible | AT2-on-convertf weights[se]/p/feasible | MATCH? (steppe vs AT2 within tier — the user\u0027s win condition) | the Haak-2015 expectation. (2) The HEADLINE: did steppe recover the Haak story — Corded Ware majority-Yamnaya (~70-80%)? the 3-way WHG/EEF/Steppe European model feasible? (3) steppe-vs-AT2 PARITY verdict (the residual block-partition ~0.3% gap is expected; flag if anything is larger). (4) the SNP counts + any caveat (maxmiss=0 vs Haak allsnps). Use Write. Return the headline + the per-model steppe-vs-AT2 match at a glance.',
