...
const sweep = await tryAgent([
  ...
  'STEPS: (1) sync + ensure the RELEASE build: run ' + RSYNC + ' then ' + ENSURE_BUILD + ' and CONFIRM BUILD_TYPE=Release (if Debug, STOP — debug per-kernel sync voids timing). (2) Confirm the 1240K geometry on the box: wc -l the .snp and .ind under ' + PREFIX + ', and xxd the first 8 bytes of the .geno (expect magic TGENO). (3) From the .ind 3rd column, build 4 NESTED real pop sets of about 5, 15, 30, 60 pops (each pop with at least 5 individuals; anchor on the Haak set, add more real v66 pops). (4) For EACH set, time extract-f2 wrapped in /usr/bin/time -v, invoking: ' + BIN + ' extract-f2 --prefix ' + PREFIX + ' --pops <set> --out /workspace/data/1240k_sweep/f2_<n> --blgsize 0.05 --maxmiss 0 --auto-only — record Elapsed wall, SNPs kept, blocks, MaxRSS; poll nvidia-smi (memory.used, -l 1) in the background during the ~60-pop run for peak VRAM. (5) Time a qpadm fit wrapped in /usr/bin/time -v: ' + BIN + ' qpadm --f2-dir <a 1240k f2 dir> --target <T> --left <a,b> --right <r..> — single model, plus a larger nr if feasible. (6) Time a multi-model rotation (qpadm-rotate if built, else loop qpadm over 20-50 target/source combos) for models/sec. Clear core dumps after. Report the structured sweep — REAL numbers only, Release build confirmed. If steppe OOMs or fails at 1240K size, report exactly where (that is itself a valuable finding).',
