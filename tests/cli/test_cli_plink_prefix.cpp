// tests/cli/test_cli_plink_prefix.cpp
//
// M-FR PLINK CLI AUTO-COVER GATE (docs/design/format-readers.md §5.3): the FULL PLINK
// (.bed/.bim/.fam, SNP-major 2-bit LSB-first) reader works THROUGH the production CLI
// and matches the convertf-PA (GENO, SNP-major) reader, ON THE GPU, on the REAL AADR —
// with NO new golden.
//
// The mechanism: run the SAME tool TWICE — once with `--prefix <fixtures_eig/v66_fit9_ped>`
// (the PLINK prefix that USED TO THROW at the GenoReader ctor's "unrecognized magic") and
// once with `--prefix <fixtures_eig/v66_fit9_pa>` (the convertf-PA prefix of the SAME 9-pop
// subset) — and assert the PLINK run (1) WORKS (exit 0, no throw) and (2) produces the SAME
// result as the PA run. Both fixtures are convertf outputs from the SAME PA source with the
// SAME poplist, so they are the SAME genotypes two ways (PLINK 2-bit LSB-first vs packed PA)
// with identical SNP/individual order; a correct PLINK reader (the LUT + bit-order flip +
// ref:=A1 polarity) MUST reproduce the PA result. Because Tier-1
// (test_plink_decode_equivalence) already proves the canonical tiles + decoded Q/V/N are
// bit-identical, EVERY downstream statistic is auto-covered — this gate proves that holds
// end-to-end through the REAL CLI binary. Since PA==TGENO==goldens (test_cli_pa_prefix),
// PLINK is transitively pinned to the goldens.
//
// THE --prefix RESOLUTION: the PLINK prefix has NO .geno, so resolve_genotype_triple picks
// the .bed/.bim/.fam siblings (a .geno-present prefix still resolves to .geno/.snp/.ind, so
// the EIGENSTRAT-family prefixes are unaffected). This test therefore ALSO exercises the
// format-aware prefix expansion + the read_bim/read_fam parser dispatch.
//
// TWO TOOLS, EACH PLINK-vs-PA:
//   (1) extract-f2 --prefix: assert PED exit==0 (the throw is GONE) AND the written f2.bin
//       is BYTE-FOR-BYTE identical to the PA run's f2.bin (STPF2BK1 deterministic).
//   (2) qpdstat --prefix: assert PED exit==0 AND est/se/z match the PA run row-for-row
//       (rtol 1e-6) — the genotype-path normalized-D.
//
// PLAIN C++ host TU (NO CUDA header): it spawns the steppe binary; the GPU work runs in the
// child. SKIPs cleanly (exit 0) when no CUDA device is visible OR the fixtures_eig prefixes
// are absent (they live only on box5090). Self-checking main(); CTest gates on the exit
// code. NO SYNTHETIC DATA.
#include <array>
#include <cmath>
#include <cstdio>
#include <cstdlib>
...
void check_close(const char* what, double got, double want, double rtol, double atol) {
    ...
    if (!ok)
        std::printf("  [FAIL] %-40s ped=% .12e pa=% .12e |d|=% .3e tol=% .3e\n",
...
int main(int argc, char** argv) {
    ...
    const std::string ped_prefix = root + "/fixtures_eig/v66_fit9_ped";  // PLINK (.bed/.bim/.fam) — the new path
    const std::string pa_prefix = root + "/fixtures_eig/v66_fit9_pa";    // convertf PA (GENO) — the oracle

    std::printf("=== M-FR PLINK CLI auto-cover: PLINK --prefix == PA(GENO) --prefix (ON THE GPU) ===\n");
    std::printf("steppe:     %s\nPED prefix: %s\nPA prefix:  %s\n", bin.c_str(),
                ped_prefix.c_str(), pa_prefix.c_str());

    if (!std::filesystem::exists(bin)) {
        std::printf("RESULT: FAIL (steppe binary not found: %s)\n", bin.c_str());
        return 1;
    }
    auto have_plink = [](const std::string& pfx) {
        return std::filesystem::exists(pfx + ".bed") && std::filesystem::exists(pfx + ".bim") &&
               std::filesystem::exists(pfx + ".fam");
    };
    auto have_eig = [](const std::string& pfx) {
        return std::filesystem::exists(pfx + ".geno") && std::filesystem::exists(pfx + ".snp") &&
               std::filesystem::exists(pfx + ".ind");
    };
    if (!have_plink(ped_prefix) || !have_eig(pa_prefix)) {
        std::printf("\nRESULT: SKIP (PLINK or PA fixture absent: %s.{bed,bim,fam} / %s.{geno,snp,ind})\n",
                    ped_prefix.c_str(), pa_prefix.c_str());
        return 0;
    }
    ...
    const std::filesystem::path tmp = std::filesystem::temp_directory_path(ec) /
        ("steppe_cli_plink_prefix_" + std::to_string(static_cast<long long>(
    ...
    // ====================================================================== (1) extract-f2
    // The PLINK prefix USED TO THROW at the GenoReader ctor (unrecognized magic). Assert it
    // now WORKS (exit 0) AND the f2.bin is byte-identical to the PA run's.
    std::printf("\n--- (1) extract-f2 --prefix: PLINK WORKS + f2.bin == PA -------------------\n");
    {
        const std::filesystem::path ped_dir = tmp / "f2_ped";
        const std::filesystem::path pa_dir = tmp / "f2_pa";
        std::vector<std::string> ped_args = {
            "extract-f2", "--prefix", ped_prefix, "--pops", pops9_arg,
            "--blgsize", blgsize, "--maxmiss", "0", "--out", ped_dir.string()};
        std::vector<std::string> pa_args = {
            "extract-f2", "--prefix", pa_prefix, "--pops", pops9_arg,
            "--blgsize", blgsize, "--maxmiss", "0", "--out", pa_dir.string()};
        const RunResult ped = run_steppe(bin, ped_args, tmp, "ef2_ped");
        if (looks_like_no_gpu(ped.text)) {
            std::printf("RESULT: SKIP (no CUDA device — extract-f2 GPU path cannot run)\n");
            return 0;
        }
        const RunResult pa = run_steppe(bin, pa_args, tmp, "ef2_pa");
        check_eq_int("extract-f2 PLINK prefix exit == 0 (the throw is GONE)", ped.exit_code, 0);
        check_eq_int("extract-f2 PA prefix exit == 0", pa.exit_code, 0);
        if (ped.exit_code != 0) std::printf("  PED stderr:\n%s\n", ped.text.c_str());
        const std::vector<char> eb = read_file_bytes(ped_dir / "f2.bin");
        const std::vector<char> pb = read_file_bytes(pa_dir / "f2.bin");
        check_true("PLINK f2.bin non-empty", !eb.empty());
        check_true("PA f2.bin non-empty", !pb.empty());
        check_true("PLINK f2.bin size == PA f2.bin size", eb.size() == pb.size());
        const bool eq = !eb.empty() && eb.size() == pb.size() &&
                        std::memcmp(eb.data(), pb.data(), eb.size()) == 0;
        check_true("extract-f2 PLINK f2.bin BIT-IDENTICAL to PA f2.bin", eq);
        std::printf("  f2.bin: PED %zu B, PA %zu B\n", eb.size(), pb.size());
    }

    // ====================================================================== (2) qpdstat
    // Genotype-path normalized-D: a few quadruples over the 9-pop union. PED == PA
    // row-for-row (rtol 1e-6).
    std::printf("\n--- (2) qpdstat --prefix: PLINK est/se/z == PA ----------------------------\n");
    {
        ...
        const RunResult ped = run_steppe(bin, dstat_args(ped_prefix), tmp, "dstat_ped");
        const RunResult pa = run_steppe(bin, dstat_args(pa_prefix), tmp, "dstat_pa");
        check_eq_int("qpdstat PLINK prefix exit == 0", ped.exit_code, 0);
        check_eq_int("qpdstat PA prefix exit == 0", pa.exit_code, 0);
        if (ped.exit_code != 0) std::printf("  PED stderr:\n%s\n", ped.text.c_str());
        const std::map<std::string, DRow> em = parse_dstat(ped.text);
        const std::map<std::string, DRow> pm = parse_dstat(pa.text);
        check_true("qpdstat parsed >=1 PLINK row", !em.empty());
        check_true("qpdstat PLINK row count == PA row count", em.size() == pm.size());
        for (const auto& [k, pr] : pm) {
            auto it = em.find(k);
            if (it == em.end()) { check_true(("qpdstat PLINK has row " + k).c_str(), false); continue; }
