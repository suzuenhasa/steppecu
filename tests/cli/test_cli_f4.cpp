// tests/cli/test_cli_f4.cpp
//
// STANDALONE f4 acceptance gate (fit-engine §6): the `steppe f4` CLI reproduces the
// REGENERATED, fixture-matched AT2 f4 golden (golden_fit0_f4_readf2.csv, 60 quartet rows)
// THROUGH the CLI, ON THE GPU. The golden was made via admixtools::f4(read_f2(the 9-pop
// maxmiss=0 f2 fixture)) so a standalone f4 over the SAME committed fixture matches it at
// tolerance (the old golden_fit0_f4.csv carried the ALLSNPS/maximal-overlap qpadm-internal
// normalization, a constant 1.1131993890x off the read_f2 fixture — a standalone f4 over
// the fixture CANNOT match THAT, which is why the regenerated golden exists).
//
// NO SYNTHETIC DATA (memory real-data-only): it writes the COMMITTED real 9-pop golden f2
// fixture (fixtures/f2_fit0_9pop.bin — the SAME tensor golden_fit0 / test_qpadm / qpwave
// use) into a temp f2-dir (STPF2BK1 f2.bin + pops.txt + meta.json), then runs the BUILT
// `steppe f4` binary for ALL 60 golden quartets (the row-aligned --pop1/--pop2/--pop3/
// --pop4 columns) and diffs the est/se/z/p table row-for-row.
//
// TIERS: est/se/z/p ALL TIGHT at rtol 1e-6 (atol 1e-9) — the regen cross-check measured
// max rel delta 1.36e-12 between the steppe assemble + the AT2 read_f2 f4, so 1e-6 is a
// comfortable gate. status == "ok".
//
// PLAIN C++ host TU (NO CUDA header): it spawns the steppe binary and parses stdout; the
// GPU work happens inside that child. SKIPs cleanly (exit 0) when no CUDA device is visible
// — identical to test_cli_qpwave / test_cli_qpadm. The harness helpers (read_raw_fixture/
// write_f2_dir/run_steppe/looks_like_no_gpu/split_csv_line/cell_d) are reused verbatim from
// the test_cli_qpwave pattern. Self-checking main(); CTest gates on the exit code.
...
// ============================================================================
// Harness — reused verbatim from test_cli_qpwave.cpp / test_cli_qpadm.cpp.
// ============================================================================
struct RawF2 {
    int P = 0, nb = 0;
...
bool write_f2_dir(const std::filesystem::path& dir, const RawF2& raw,
    ...
    mf << "{\n  \"format\": \"STPF2BK1\",\n  \"P\": " << raw.P
       << ",\n  \"n_block\": " << raw.nb
       << ",\n  \"provenance\": \"test fixture (real-AADR 9-pop f4 golden f2)\"\n}\n";
    mf.close();
    return true;
}
...
RunResult run_steppe(const std::string& bin, const std::vector<std::string>& args,
    ...
    RunResult rr;
    const std::filesystem::path outf = tmp / "cli_f4_stdout.txt";
    std::string cmd = "\"" + bin + "\"";
...
// One golden quartet row: the four pop names + est/se/z/p.
struct F4Row {
    std::string p1, p2, p3, p4;
    double est = 0.0, se = 0.0, z = 0.0, p = 0.0;
};

// Parse golden_fit0_f4_readf2.csv (header pop1,pop2,pop3,pop4,est,se,z,p + 60 data rows).
bool read_golden_f4(const std::string& path, std::vector<F4Row>& out) {
    std::ifstream f(path);
    if (!f) { std::printf("  [SKIP] golden f4 csv absent: %s\n", path.c_str()); return false; }
    std::string line;
    bool header = true;
...
// Map the steppe f4 CSV output (header pop1..pop4,est,se,z,p + N data rows) keyed on the
// quartet name 4-tuple, so the diff is order-independent (the steppe order == input order
// == golden order, but key on names for robustness).
std::map<std::string, F4Row> parse_steppe_f4(const std::string& text) {
    std::map<std::string, F4Row> out;
...
int main(int argc, char** argv) {
    ...
    std::printf("=== standalone f4 CLI parity (THROUGH the CLI, ON THE GPU) ===\n");
    ...
    // ---- the committed 9-pop f2 fixture (the SAME tensor golden_fit0 / qpwave use) ----
    RawF2 raw;
    if (!read_raw_fixture(golden_dir + "/fixtures/f2_fit0_9pop.bin", raw)) {
        std::printf("\nRESULT: SKIP (f4 fixture absent)\n");
        return 0;  // absent fixture is a clean skip (like the parity test)
    }

    // ---- the regenerated fixture-matched golden (60 genuine quartets) -----------------
    std::vector<F4Row> golden;
    if (!read_golden_f4(golden_dir + "/csv/golden_fit0_f4_readf2.csv", golden)) {
        std::printf("\nRESULT: SKIP (regenerated f4 golden absent)\n");
        return 0;
    }
    check_true("golden has 60 quartet rows", golden.size() == 60);
    ...
    const std::filesystem::path tmp_root =
        std::filesystem::temp_directory_path(ec) / ("steppe_cli_f4_test_" + std::to_string(
    ...
    const std::filesystem::path dir = tmp_root / "f4";
    ...
    // Build the row-aligned --pop1/--pop2/--pop3/--pop4 columns for ALL 60 golden quartets.
    std::vector<std::string> c1, c2, c3, c4;
    for (const F4Row& g : golden) {
        c1.push_back(g.p1); c2.push_back(g.p2); c3.push_back(g.p3); c4.push_back(g.p4);
    }

    // ---------------- CSV run (the full 60-quartet batch) ----------------
    const RunResult csv = run_steppe(steppe_bin,
        {"f4", "--f2-dir", dir.string(),
    ...
    const std::map<std::string, F4Row> got = parse_steppe_f4(csv.stdout_text);
    ...
    for (const F4Row& g : golden) {
        ...
        if (it == got.end()) {
            std::printf("  [FAIL] quartet missing from steppe output: %s\n", key.c_str());
            ++g_failures;
            continue;
        }
        const F4Row& s = it->second;
        char nm[160];
    ...
    // ---------------- single-quartet --pops convenience (the SANITY) ----------------
    // `steppe f4 --pops England_BellBeaker,Czechia_EBA_CordedWare,Han,Iran_GanjDareh_N`
    // matches golden row 1 (the first golden quartet).
    {
        const F4Row& g0 = golden.front();
        const std::string pops_arg = g0.p1 + "," + g0.p2 + "," + g0.p3 + "," + g0.p4;
        const RunResult one = run_steppe(steppe_bin,
            {"f4", "--f2-dir", dir.string(), "--pops", pops_arg,
             "--format", "csv", "--device", "0"},
            tmp_root);
        check_eq_int("single-quartet exit code == 0", one.exit_code, 0);
        const std::map<std::string, F4Row> g1 = parse_steppe_f4(one.stdout_text);
        check_eq_int("single-quartet 1 row", static_cast<long long>(g1.size()), 1);
        const std::string key = g0.p1 + "|" + g0.p2 + "|" + g0.p3 + "|" + g0.p4;
        const auto it = g1.find(key);
        check_true("single-quartet matches golden row 1", it != g1.end());
        if (it != g1.end()) {
    ...
    if (g_failures == 0) {
        std::printf("\nRESULT: PASS (60 f4 quartets reproduced through the GPU; CSV + JSON + --pops)\n");
        return 0;
    }
