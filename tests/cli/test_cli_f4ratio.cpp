// tests/cli/test_cli_f4ratio.cpp
//
// STANDALONE f4-ratio acceptance gate (fit-engine §6): the `steppe f4-ratio` CLI reproduces
// the fixture-matched AT2 qpf4ratio golden (golden_fit0_f4ratio_readf2.csv, 10 5-tuple rows)
// THROUGH the CLI, ON THE GPU. The FIVE-column clone of test_cli_f3.cpp / test_cli_f4.cpp. The
// golden was made via admixtools::qpf4ratio(read_f2(the 9-pop maxmiss=0 f2 fixture)) so a
// standalone f4-ratio over the SAME committed fixture matches it at tolerance.
//
...
// f3 use) into a temp f2-dir (STPF2BK1 f2.bin + pops.txt + meta.json), then runs the BUILT
// `steppe f4-ratio` binary for ALL 10 golden 5-tuples (the row-aligned --pop1..--pop5 columns)
// and diffs the alpha/se/z table row-for-row.
//
// TIERS: alpha/se/z ALL TIGHT at rtol 1e-6 (atol 1e-9). status == "ok".
//
...
// harness helpers are reused verbatim from the test_cli_f3 pattern. Self-checking main();
...
void check_close(const char* what, double got, double want, double rtol, double atol) {
    ...
    if (!ok)
        std::printf("  [FAIL] %-40s got=% .12e want=% .12e |d|=% .3e tol=% .3e\n",
...
// ============================================================================
// Harness — reused verbatim from test_cli_f3.cpp / test_cli_f4.cpp.
// ============================================================================
struct RawF2 {
    int P = 0, nb = 0;
...
bool write_f2_dir(const std::filesystem::path& dir, const RawF2& raw,
    ...
    mf << "{\n  \"format\": \"STPF2BK1\",\n  \"P\": " << raw.P
       << ",\n  \"n_block\": " << raw.nb
       << ",\n  \"provenance\": \"test fixture (real-AADR 9-pop f4-ratio golden f2)\"\n}\n";
    mf.close();
    return true;
}
...
RunResult run_steppe(const std::string& bin, const std::vector<std::string>& args,
    ...
    RunResult rr;
    const std::filesystem::path outf = tmp / "cli_f4ratio_stdout.txt";
    std::string cmd = "\"" + bin + "\"";
...
// One golden 5-tuple row: the five pop names + alpha/se/z.
struct F4RRow {
    std::string p1, p2, p3, p4, p5;
    double alpha = 0.0, se = 0.0, z = 0.0;
};

// Parse golden_fit0_f4ratio_readf2.csv (header pop1..pop5,alpha,se,z + 10 data rows).
bool read_golden_f4ratio(const std::string& path, std::vector<F4RRow>& out) {
    std::ifstream f(path);
    if (!f) { std::printf("  [SKIP] golden f4-ratio csv absent: %s\n", path.c_str()); return false; }
    std::string line;
    bool header = true;
    ...
    while (std::getline(f, line)) {
        ...
        r.p4 = cells[col["pop4"]];
        r.p5 = cells[col["pop5"]];
        r.alpha = cell_d(cells[col["alpha"]]);
...
// Map the steppe f4-ratio CSV output (header pop1..pop5,alpha,se,z + N data rows) keyed on the
// 5-tuple name 5-tuple, so the diff is order-independent.
std::map<std::string, F4RRow> parse_steppe_f4ratio(const std::string& text) {
    std::map<std::string, F4RRow> out;
    ...
    while (std::getline(ss, line)) {
        ...
        if (!col.count("pop1") || !col.count("alpha")) continue;
        F4RRow r;
        r.p1 = cells[col["pop1"]];
        r.p2 = cells[col["pop2"]];
        r.p3 = cells[col["pop3"]];
        ...
        r.se    = cell_d(cells[col["se"]]);
        r.z     = cell_d(cells[col["z"]]);
        out[r.p1 + "|" + r.p2 + "|" + r.p3 + "|" + r.p4 + "|" + r.p5] = r;
    }
    return out;
}

std::string key_of(const F4RRow& r) {
    return r.p1 + "|" + r.p2 + "|" + r.p3 + "|" + r.p4 + "|" + r.p5;
}
...
int main(int argc, char** argv) {
    ...
    std::printf("=== standalone f4-ratio CLI parity (THROUGH the CLI, ON THE GPU) ===\n");
    ...
    // ---- the committed 9-pop f2 fixture (the SAME tensor golden_fit0 / qpwave / f4 / f3 use) ---
    RawF2 raw;
    if (!read_raw_fixture(golden_dir + "/fixtures/f2_fit0_9pop.bin", raw)) {
        std::printf("\nRESULT: SKIP (f4-ratio fixture absent)\n");
        return 0;  // absent fixture is a clean skip (like the parity test)
    }

    // ---- the fixture-matched golden (10 genuine 5-tuples) -----------------------------
    std::vector<F4RRow> golden;
    if (!read_golden_f4ratio(golden_dir + "/csv/golden_fit0_f4ratio_readf2.csv", golden)) {
        std::printf("\nRESULT: SKIP (f4-ratio golden absent)\n");
        return 0;
    }
    check_true("golden has 10 5-tuple rows", golden.size() == 10);
    ...
    const std::filesystem::path tmp_root =
        std::filesystem::temp_directory_path(ec) / ("steppe_cli_f4ratio_test_" + std::to_string(
    ...
    const std::filesystem::path dir = tmp_root / "f4ratio";
    ...
    // Build the row-aligned --pop1..--pop5 columns for ALL 10 golden 5-tuples.
    std::vector<std::string> c1, c2, c3, c4, c5;
    for (const F4RRow& g : golden) {
        c1.push_back(g.p1); c2.push_back(g.p2); c3.push_back(g.p3);
        c4.push_back(g.p4); c5.push_back(g.p5);
    }

    // ---------------- CSV run (the full 10-tuple batch) ----------------
    const RunResult csv = run_steppe(steppe_bin,
        {"f4-ratio", "--f2-dir", dir.string(),
         "--pop1", join(c1), "--pop2", join(c2), "--pop3", join(c3),
         "--pop4", join(c4), "--pop5", join(c5),
         "--format", "csv", "--device", "0"},
        tmp_root);
    if (looks_like_no_gpu(csv.stdout_text)) {
        std::printf("  [SKIP] no CUDA device visible — CLI GPU f4-ratio cannot run\n");
        std::filesystem::remove_all(tmp_root, ec);
    ...
    const std::map<std::string, F4RRow> got = parse_steppe_f4ratio(csv.stdout_text);
    check_eq_int("steppe emitted 10 rows", static_cast<long long>(got.size()), 10);

    for (const F4RRow& g : golden) {
        const std::string key = key_of(g);
        const auto it = got.find(key);
        if (it == got.end()) {
            std::printf("  [FAIL] 5-tuple missing from steppe output: %s\n", key.c_str());
            ++g_failures;
            continue;
        }
        const F4RRow& s = it->second;
        char nm[200];
        std::snprintf(nm, sizeof(nm), "alpha %s", key.c_str());
        check_close(nm, s.alpha, g.alpha, 1e-6, 1e-9);
    ...
    // ---------------- single-tuple --pops convenience (the SANITY) ----------------
    {
        const F4RRow& g0 = golden.front();
        const std::string pops_arg =
            g0.p1 + "," + g0.p2 + "," + g0.p3 + "," + g0.p4 + "," + g0.p5;
        const RunResult one = run_steppe(steppe_bin,
            {"f4-ratio", "--f2-dir", dir.string(), "--pops", pops_arg,
             "--format", "csv", "--device", "0"},
            tmp_root);
        check_eq_int("single-tuple exit code == 0", one.exit_code, 0);
        const std::map<std::string, F4RRow> g1 = parse_steppe_f4ratio(one.stdout_text);
        check_eq_int("single-tuple 1 row", static_cast<long long>(g1.size()), 1);
        const std::string key = key_of(g0);
        const auto it = g1.find(key);
        check_true("single-tuple matches golden row 1", it != g1.end());
        if (it != g1.end()) {
            check_close("--pops alpha", it->second.alpha, g0.alpha, 1e-6, 1e-9);
    ...
    const RunResult js = run_steppe(steppe_bin,
        {"f4-ratio", "--f2-dir", dir.string(),
         "--pop1", c1[0], "--pop2", c2[0], "--pop3", c3[0], "--pop4", c4[0], "--pop5", c5[0],
         "--format", "json", "--device", "0"},
        tmp_root);
    check_eq_int("json exit code == 0", js.exit_code, 0);
    {
        const std::string& t = js.stdout_text;
        const std::size_t ek = t.find("\"alpha\"");
        check_true("json has alpha field", ek != std::string::npos);
        if (ek != std::string::npos) {
            const std::size_t colon = t.find(':', ek);
            const double alpha = std::strtod(t.c_str() + colon + 1, nullptr);
            check_close("json alpha[0]", alpha, golden.front().alpha, 1e-6, 1e-9);
        }
    ...
    if (g_failures == 0) {
        std::printf("\nRESULT: PASS (10 f4-ratio 5-tuples reproduced through the GPU; CSV + JSON + --pops)\n");
        return 0;
    }
