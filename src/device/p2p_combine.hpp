...
// (a byte-exact DMA copy) and PLACES them in the same fixed g = 0..G-1 order
...
// canAccessPeer==1 both directions on rtxbox) and PLACES it on-device into its
...
// each device's compute_f2_blocks produced — and the on-device step PLACES each
// partial in the SAME FIXED g=0..G-1 ORDER into its disjoint result slice (a raw byte
// copy, each slab written exactly once — NO sum, NO cudaMemset; see the
// combine_f2_partials_resident doc-comment below), the
// identical fixed-order placement combine_f2_partials_host performs on the host. "the
// transport only moves bytes; software fixes the order" — so the two combine tiers are
// parity-NEUTRAL siblings, and both equal the single-GPU reference. NEVER an NCCL
