...
//
// The OPT-IN device-resident P2P f2 combine — CUDA-FREE DECLARATION of the single-node
// multi-GPU (SPMG) fast-path combine: GPU 0 (the combine root) pulls each peer's compact
// partial via cudaMemcpyPeer and places it on-device in the fixed g=0..G-1 order.
// Design (CUDA-free contract + transport rationale): see docs/design/p2p-combine.md
// (architecture.md §11.4, §12).
//
// PARITY INVARIANT (§11.4, §12): the device-resident combine is bit-identical to the
// host-staged combine and the single-GPU reference — the transport only moves bytes,
// software fixes the order. NEVER an NCCL AllReduce.
