...
//
// The GPU genotype-path normalized-D per-SNP reduction (qpDstat Part B + the qpfstats
// genotype-f4 numerator reduce). Statistic derivation + the SNP-tiled pairwise-difference
// reuse design + the huge-npop fallback: see docs/design/dstat-kernel.md
// (architecture.md §12, §13).
//
// PARITY INVARIANT (§12 cancellation carve-out): this kernel is golden-exact — each
// (combo, block) num/den sum is BIT-IDENTICAL (not merely within rtol) to the legacy
// per-(combo,block) kernel, in NATIVE FP64 (never emulated).
//
// CUDA TU: PRIVATE to steppe_device (architecture.md §4); the kernel body + <<<>>> live
// ONLY here, reached through the narrow launch wrapper dstat_kernel.cuh (§7).
