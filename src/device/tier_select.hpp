...
#include "steppe/config.hpp"       // kResidentTierVramFraction, kHostTierRamFraction, kCublasWorkspaceBytes, kFeederRawBufsPerPop, kFeederOutBufsPerPop, kStreamDeviceChunks, DeviceConfig::ForceTier
...
/// it must keep the (kFeederRawBufsPerPop+kFeederOutBufsPerPop)·P·M term exactly.
[[nodiscard]] inline std::size_t resident_working_set_bytes(int P, long M) noexcept {
    ...
    // 7·P·M = raw inputs (kFeederRawBufsPerPop=3) + feeder outputs
    // (kFeederOutBufsPerPop=4); named in config.hpp so this policy coefficient
    // and the streamed-path one below cannot drift from the real feeder malloc.
...
///   device ring            kStreamDeviceChunks·P²·max_nb   (the f2/vpair ring buffers; config.hpp)
