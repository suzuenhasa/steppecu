...
F2BlockTensor compute_f2_blocks_multigpu(
    ...
    // THE §4 COMBINE GATE — see docs/design/multigpu-combine-gate.md (architecture.md §11.4, §12).
    // The four-term predicate is single-homed in select_p2p_combine (above); both the P2P and the
    // host-staged combine tiers are bit-identical to each other and to the single-GPU reference
    // (parity-NEUTRAL — the transport only moves bytes; software fixes the g=0..G-1 order; §12).
