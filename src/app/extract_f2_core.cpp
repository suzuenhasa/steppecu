...
F2ExtractResult run_extract_f2(const std::string& geno,
    ...
    // Pop-coverage maxmiss (AT2 population-axis): drop SNP s if the fraction of selected
    // pops with N(pop,s)==0 (no data) exceeds maxmiss. maxmiss>=1 keeps every SNP;
    // maxmiss==0 is the global intersection (the golden_fit0 SNP set).
    ...
    const long M_kept = static_cast<long>(n_kept);
    if (M_kept <= 0) {
        throw std::invalid_argument(
    ...
    // aligned for assign_blocks; cmd_extract_f2.cpp:427-449).
    std::vector<double> Qk(static_cast<std::size_t>(P) * n_kept, 0.0);
    std::vector<double> Vk(static_cast<std::size_t>(P) * n_kept, 0.0);
    std::vector<double> Nk(static_cast<std::size_t>(P) * n_kept, 0.0);
